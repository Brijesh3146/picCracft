...
function FontStyles() {
  ...
  return (
    <div>
      <Toggle aria-label="Toggle" defaultPressed={activeObject?.fontWeight=='bold'}
       onClick={()=>onSettingClick('bold')}>
        <Bold className='h-4 w-4' size={'lg'}/>
      </Toggle>
      <Toggle aria-label="Toggle italic" defaultPressed={activeObject?.fontStyle=='italic'}
       onClick={()=>onSettingClick('italic')}>
        <Italic className='h-4 w-4' size={'lg'}/>
      </Toggle>
      <Toggle aria-label="Toggle underline" defaultPressed={activeObject?.underline=='underline'}
